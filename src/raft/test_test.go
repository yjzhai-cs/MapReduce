...
func TestInitialElection2A(t *testing.T) {
	...
	utils.Debugger(utils.DLog, "Test (2A): initial election")
	...
	if term1 < 1 {
		...
		utils.Debugger(utils.DError, "term is %v, but should be at least 1", term1)
	}
	...
	if term1 != term2 {
		...
		utils.Debugger(utils.DWarn, "warning: term changed even though there were no failures")
	}
...
func TestReElection2A(t *testing.T) {
	...
	utils.Debugger(utils.DLog, "Test (2A): election after network failure")

	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	utils.Debugger(utils.DLog, " S%d disconnect.", leader1)
	...
	utils.Debugger(utils.DLog, " S%d reconnect.", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	utils.Debugger(utils.DLog, " S%d and S%d disconnect.", leader2, (leader2 + 1) % servers)
	...
	utils.Debugger(utils.DLog, " S%d reconnect.", (leader2 + 1) % servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	utils.Debugger(utils.DLog, " S%d reconnect.", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	utils.Debugger(utils.DLog, "Test (2A): multiple elections")
	cfg.checkOneLeader()
	...
	for ii := 1; ii < iters; ii++ {
		...
		i2 := rand.Int() % servers
		i3 := rand.Int() % servers
		utils.Debugger(utils.DLog, " S%d,S%d and S%d disconnect.", i1,i2,i3)
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		...
		utils.Debugger(utils.DLog, " S%d,S%d and S%d reconnect.", i1,i2,i3)
		cfg.connect(i1)
		cfg.connect(i2)
		cfg.connect(i3)
	}
...
func TestFollowerFailure2B(t *testing.T) {
	...
	utils.Debugger(utils.DLog, " S%d disconnect.", (leader1 + 1) % servers)
	...
	utils.Debugger(utils.DLog, " S%d and S%d disconnect.", (leader2 + 1) % servers, (leader2 + 2) % servers)
...
func TestLeaderFailure2B(t *testing.T) {
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	utils.Debugger(utils.DLog, " S%d disconnect.", leader2)
...
func TestFailAgree2B(t *testing.T) {
	...
	utils.Debugger(utils.DLog, " S%d disconnect.", (leader + 1) % servers)
	...
	utils.Debugger(utils.DLog, " S%d reconnect.", (leader + 1) % servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	utils.Debugger(utils.DLog, " S%d, S%d and S%d disconnect.", (leader + 1) % servers, (leader + 2) % servers, (leader + 3) % servers)
	...
	utils.Debugger(utils.DLog, " S%d, S%d and S%d reconnect.", (leader + 1) % servers, (leader + 2) % servers, (leader + 3) % servers)
...
func TestBackup2B(t *testing.T) {
	...
	utils.Debugger(utils.DLog, " S%d,S%d and S%d disconnect.", (leader1 + 2) % servers,(leader1 + 3) % servers,(leader1 + 4) % servers)
	...
	cfg.disconnect((leader1 + 1) % servers)
	utils.Debugger(utils.DLog, " S%d and S%d disconnect.", (leader1 + 0) % servers,(leader1 + 1) % servers)
	...
	utils.Debugger(utils.DLog, " S%d,S%d and S%d reconnect.", (leader1 + 2) % servers,(leader1 + 3) % servers,(leader1 + 4) % servers)
	...
	utils.Debugger(utils.DLog, " S%d disconnect.", other)
	...
	utils.Debugger(utils.DLog, " all the servers disconnect.")


	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)
	utils.Debugger(utils.DLog, " S%d,S%d and S%d reconnect.", (leader1 + 0) % servers, (leader1 + 1) % servers, other)
	...
	utils.Debugger(utils.DLog, " all the servers reconnect.")
	cfg.one(rand.Int(), servers, true)
...
func TestPersist12C(t *testing.T) {
	...
	utils.Debugger(utils.DLog, " all servers carsh and re-start")
	...
	leader1 := cfg.checkOneLeader()
	utils.Debugger(utils.DLog, " S%d carshes and re-starts", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	utils.Debugger(utils.DLog, " S%d carshes and re-starts", leader2)
	cfg.disconnect(leader2)
	...
	utils.Debugger(utils.DLog, " S%d carshes and re-starts", i3)
	cfg.disconnect(i3)
...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		...
		cfg.disconnect((leader1 + 1) % servers)
		cfg.disconnect((leader1 + 2) % servers)
		utils.Debugger(utils.DLog, " S%d and S%d disconnect.", (leader1 + 1) % servers,(leader1 + 2) % servers)
		...
		cfg.disconnect((leader1 + 3) % servers)
		cfg.disconnect((leader1 + 4) % servers)
		utils.Debugger(utils.DLog, " S%d,S%d and S%d disconnect.", (leader1 + 0) % servers,(leader1 + 3) % servers,(leader1 + 4) % servers)
		...
		cfg.connect((leader1 + 1) % servers)
		cfg.connect((leader1 + 2) % servers)
		utils.Debugger(utils.DLog, " S%d and S%d reconnect.", (leader1 + 1) % servers,(leader1 + 2) % servers)

		time.Sleep(RaftElectionTimeout)

		cfg.start1((leader1+3)%servers, cfg.applier)
		cfg.connect((leader1 + 3) % servers)
		utils.Debugger(utils.DLog, " S%d reconnect.", (leader1 + 3) % servers)
		...
		cfg.connect((leader1 + 4) % servers)
		cfg.connect((leader1 + 0) % servers)
		utils.Debugger(utils.DLog, " S%d and S%d reconnect.", (leader1 + 0) % servers,(leader1 + 4) % servers)
	}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			utils.Debugger(utils.DLog, " S%d disconnect.", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
			utils.Debugger(utils.DLog, " S%d crash.", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		...
		if disconnect {
			...
			cfg.connect(victim)
			utils.Debugger(utils.DLog, " S%d reconnect.", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			...
			cfg.connect(victim)
			utils.Debugger(utils.DLog, " S%d restart due to crash.", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
	}
