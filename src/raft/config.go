...
import "runtime"
import "math/rand"
import crand "crypto/rand" // crypto包含各种常用的加密算法和随机数生成器
import "math/big" // big实现了各种大整数运算
import "encoding/base64"
import "time"
import "fmt"

// 生成一个指定长度的字符串,用作端口文件名
func randstring(n int) string {
	b := make([]byte, 2*n)
	crand.Read(b) // 向b中写入2*n个随机数
...
func make_config(t *testing.T, n int, unreliable bool, snapshot bool) *config {
	utils.InitLogger() // init logger
	ncpu_once.Do(func() { // 原子操作，只会执行一次
		if runtime.NumCPU() < 2 { // 返回当前系统的 CPU 核数量
			fmt.Printf("warning: only one CPU, which may conceal locking bugs\n")
		}
		rand.Seed(makeSeed()) // 设置随机种子
	})
	runtime.GOMAXPROCS(4) // 设置最大的可同时使用的 CPU 核数
	cfg := &config{}
	cfg.t = t
	...
	cfg.start = time.Now() // 当前本地时间

	cfg.setunreliable(unreliable) // 配置网络的可靠性

	cfg.net.LongDelays(true) // 网络延迟
	...
	for i := 0; i < cfg.n; i++ {
		cfg.logs[i] = map[int]interface{}{} // 分配内存空间
		cfg.start1(i, applier) // 启动server 1
	}

	// connect everyone
	for i := 0; i < cfg.n; i++ {
		cfg.connect(i) // server 及其相关的clientend 接入net
	}
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	for j := 0; j < cfg.n; j++ {
		ends[j] = cfg.net.MakeEnd(cfg.endnames[i][j]) // create an client end named cfg.endnames[i][j]
		cfg.net.Connect(cfg.endnames[i][j], j)
	}

	cfg.mu.Lock()

	cfg.lastApplied[i] = -1 // 0
...
func (cfg *config) connect(i int) {
	...
	cfg.connected[i] = true // server i 是在net上
...
func (cfg *config) end() {
	...
	if cfg.t.Failed() == false {
		...
		fmt.Printf("  ... Passed --")
		utils.Debugger(utils.DLog, "  ... Passed -- %4.1f  %d %4d %7d %4d\n", t, npeers, nrpc, nbytes, ncmds)
